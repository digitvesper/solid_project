...
class Engine:
    ...
    def monster_attack(self, life):
        if life > 0:
            self.fighter_life = self.fighter_life - life
        if self.fighter_life <= 0:
            print("Боец побежден!")

    def fighter_attack(self, life):
        if life > 0:
            self.monster_life = self.monster_life - life
...
if __name__ == "__main__":
    ...
    game_engine.fighter_attack(fighter.attack())

    fighter.change_weapon(bow)
    game_engine.fighter_attack(fighter.attack())

    game_engine.monster_attack(monster.attack())
